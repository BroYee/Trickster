// ON_VP.cpp : 응용 프로그램에 대한 진입점을 정의합니다.
//
...
bool menu = true;//메뉴는 메뉴 화면을 보이게 하는가 안 하는가를 보여준다
	...
	_In_ int       nCmdShow)
{
	...
	DWORD dwTime = GetTickCount(); //DWORD 는 양의 정수를 받기에 좋다. GetTickCount();는 컴퓨터 부팅 된 후의 시간받는다
	DWORD dwCollipse;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		...
		ixpos = LOWORD(lParam);//마우스의 x포지션
		iypos = HIWORD(lParam);//마우스의 y포지션

		if (700 <= ixpos && ixpos <= 840 &&
			450 <= iypos && iypos <= 500 && menu == true)//만약 내가만든 게임스타트 버튼안을 클릭하게 된다면
		{
			menu = false; //menu는 메인화면을 나타내는가 안 나타내는가
			int a = 0;
			if (a == 0)
			{
			...
			510 <= iypos && iypos <= 560 && menu == true)//만약 내가만든 게임 종료 버튼안을 클릭하게 된다면
		{
			DestroyWindow(g_hWnd);//(창을 종료한다.)
		}
		else if (1325 <= ixpos && ixpos <= 1467 &&
			0 <= iypos && iypos <= 122 && menu == false)//만약 내가만든 게임 일시정지 버튼안을 클릭하게 된다면.
		{
			menu = true; //처음의 메인 화면으로 넘어가진다.(임시로 만들어 보았다)
		}
